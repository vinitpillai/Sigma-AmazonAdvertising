...
let credentialsRiakBucket = "Test"
...
let processJob (requestContainer: AmazonAdvertisingRequest)  = 
    ...
    try 
        ...
        | GetAppToken oAuthKeys -> AmazonAdvertising.GetAWSAppAccessToken oAuthKeys |> handleResponse
        | _                     -> match credentials.IsSome with
                                   ...
                                   | true  -> match requestContainer.request with 
                                              | GetAppToken oAuthKeys -> AmazonAdvertising.GetAWSAppAccessToken oAuthKeys |> handleResponse
                                              | AmazonReportRequest reportRequest -> AmazonAdvertising.CreateReport credentials.Value.AccessToken reportRequest.reportType reportRequest.reportRequest |> handleResponse
                                              | AmazonReportById reportId -> AmazonAdvertising.GetReportById credentials.Value.AccessToken reportId  |> handleResponse
...
Console.WriteLine("Starting Brandwatch API Microservice ...")
